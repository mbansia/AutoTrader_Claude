...
- **L44 — Never use direct dict indexing on `_client.markets[symbol]` in a futures-only CCXT client.** A Binance CCXT client configured with `defaultType: "future"` only loads perp markets (e.g. `LITE/USDT:USDT`). When a new token is listed on USDM futures before its spot market appears in the client's market dict, `markets["LITE/USDT"]` raises `KeyError` on every cycle, driving `cycle_error_rate_high`. Fix: use `markets.get(symbol, {})` so `lot_step / tick_size / min_notional` return safe defaults (0.001 / 0.01 / 5.0) rather than crashing the loop. 2026-05-18.
- **L45 — A healthy loop never logs events; the diagnostics anomaly fires perpetually.** `bot_events` rows are written only on errors, warnings, or exceptional conditions (naked legs, crashes). A loop running cleanly — scanning + rejecting every candidate — never touches `bot_events`. The diagnostics `no_recent_events` rule fires when `seconds_since_last_event > 3600`, so a perfectly healthy bot silently triggers a CRITICAL anomaly forever. Fix: emit one INFO "loop heartbeat" event per (mode, venue) per hour from the runner loop so `last_event_ts` stays fresh. 2026-05-23.
