...
2026-05-25 PR #77 — Test coverage: core/sizing.py 87% → 100% + web/routes/dashboard.py 89% → 100% (13 new tests: _vwap_up_to zero-qty/early-break/empty-levels, _floor_to_step zero-step, all rejection guards in buy-spot/sell-spot/perp/size_entry, transactions symbol filter, stuck-close alert render, _age_hours None guard). 167/167 tests. [directive: test coverage]
2026-05-25 PR #81 — Anomaly fix: SQLite WAL mode + busy_timeout=5000 in state/db.py build_engine(). Eliminated runner_crash "database is locked" on all 3 venues caused by PRs #79/#80 adding concurrent DB flushes (_write_balance_snapshot + _ingest_capital_flows) across 3 simultaneous runner threads. 184/184 tests. [directive: anomaly-driven bug fix]
2026-05-25 PR #82 — Test coverage: loop/cycle.py 72% → 78% (6 new tests: entry snapshot_skew rejection, entry zero-qty rejection, entry both-legs FOK rejected/REJECTED outcome, entry perp-only single-leg orphan naked, exit snapshot_skew WARN, stuck-close ERROR). 190/190 tests. [directive: test coverage]
